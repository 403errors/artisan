...
- **Phase 3.3 — Planning Agent** ✅ done
  - Consumes `DomainExpertOutput` (+ prior retry feedback if looping back), emits `Plan` (steps, touched files, **test cases**, **doc updates**). See `agents/src/artisan_agents/agents/planning_agent.py`.
  - DoD: plan output always includes non-empty `test_cases` and `doc_updates` fields when the ticket touches user-facing behavior — enforced as a post-hoc Python validation (one amended-prompt retry) when a `frontend` domain output is present, not just trusted from the model. `agents/tests/test_planning_agent.py`.
