...
| Model | `gemini-3.8-flash` | pin the explicit model id in code — never a `latest` alias |
...
- Gemini model id is always pinned explicitly in code (`gemini-3.8-flash`), never resolved via a "latest" alias.
