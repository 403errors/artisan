...
**Stage: Sprint 2 deployed; ticket-bootstrap half of Gate 1 confirmed live end-to-end; intake-evaluation half still only unit-tested.** All infra is live: Pub/Sub topic + push subscription, orchestrator on Cloud Run, GitHub App webhook pointed at it, IAM wired. Jira access was switched from `mcp-atlassian` (MCP) to direct REST API calls after a live field-test failure (see "Known follow-up" below) — **confirmed working with 6 real tickets created on project `ART`** during that debugging (`ART-1`–`ART-6`, since cleaned up: commented as test artifacts and transitioned to Done, as the service account lacks Jira delete permission). What's proven live: webhook signature verification, Pub/Sub delivery + idempotency, Firestore ticket-doc creation, and real Jira ticket creation via REST. What's **not yet** proven live (only covered by unit tests with mocks): the Intake Agent's real Gemini call, `transition_ticket`/`add_comment` against a real ticket, and `get_issue_thread` against a real GitHub issue — all the smoke tests so far used synthetic GitHub issue numbers that don't actually exist on `403errors/artisan-demo`, so that part of the pipeline 404s harmlessly rather than being exercised. A real GitHub issue is needed to close that gap.
...
## Known follow-up: Sprint 2 infra/deployment — done; ticket bootstrap verified live; intake evaluation still needs a real GitHub issue

Deployed: Pub/Sub topic + subscription, orchestrator Cloud Run service, IAM grants (`orchestrator@` → `iam.serviceAccountTokenCreator` for Pub/Sub's push-service-agent, needed for push-auth token minting — this was *not* auto-granted by `--push-auth-service-account` and had to be added explicitly), GitHub App webhook URL.

**Verified live:** webhook signature verification (401 on forged signature); Pub/Sub publish → push delivery → OIDC verification → dispatch (confirmed with a no-op `pull_request` event); ticket bootstrap end-to-end — `issues opened` → Firestore ticket doc created → **real Jira ticket created via direct REST** (6 tickets, `ART-1`–`ART-6`, during iterative debugging; cleaned up afterward).

**Not yet verified live** (unit/mock-tested only): everything past ticket bootstrap — the Intake Agent's real Gemini call, the `sufficient`/`insufficient` branch logic against real GitHub issue content, `github_client.get_issue_thread`/`post_issue_comment` against a real issue, and `jira_client.transition_ticket`/`add_comment` against a real ticket. All smoke tests so far used synthetic issue numbers (990001–990006) that don't exist on `403errors/artisan-demo`, so `get_issue_thread` 404s before the Intake Agent ever runs. **Needs a real GitHub issue opened on the demo repo to close this out** — that's the actual remaining gap, not a deployment or Jira-auth problem.
...
- The Intake Agent → clarification/sufficient branches are only unit-tested (mocked model, mocked GitHub/Jira) — not yet exercised against a real GitHub issue on the live system. Do that before considering Sprint 2 fully closed.
...
**Close out Sprint 2**: open a real GitHub issue on `403errors/artisan-demo` (one deliberately vague, to exercise the clarification path; or one well-specified, to exercise the sufficient → *In Progress* path) and confirm the full Gate 1 happy path against the live system — Intake Agent call, GitHub comment or Jira transition, Firestore state. Then **Sprint 3 — Gate 2 (Plan → Execute → Verify → PR)**, per [SPRINT.md](./SPRINT.md#sprint-3--gate-2-plan--execute--verify--pr).
