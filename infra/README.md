...
- **CI/CD is live**: `.github/workflows/ci.yml` (lint + tests + Next.js build on every push/PR) and
  `.github/workflows/deploy.yml` (rebuilds `orchestrator`, `execution-sandbox`, and `dashboard`
  images and redeploys Cloud Run on push to `main`, via Workload Identity Federation). One-time
  WIF + deployer-service-account setup and the required GitHub secrets are documented in
